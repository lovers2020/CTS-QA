...
import { Plus, ChevronLeft, ChevronRight, X, Clock, MapPin, User as UserIcon } from 'lucide-react';
...
const ScheduleManagement: React.FC<ScheduleManagementProps> = ({ user, schedules, onAddSchedule }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setShowForm(false);
    setFormData({ title: '', type: ScheduleType.MEETING, startDate: '', endDate: '', startTime: '', endTime: '', description: '' });
  };

  const getTypeColorStyles = (type: ScheduleType) => {
    switch (type) {
      case ScheduleType.MEETING: return 'bg-blue-100 text-blue-700 border-blue-200 hover:bg-blue-200';
      case ScheduleType.BUSINESS_TRIP: return 'bg-purple-100 text-purple-700 border-purple-200 hover:bg-purple-200';
      case ScheduleType.VACATION: return 'bg-emerald-100 text-emerald-700 border-emerald-200 hover:bg-emerald-200';
      case ScheduleType.REMOTE: return 'bg-amber-100 text-amber-700 border-amber-200 hover:bg-amber-200';
      case ScheduleType.PERSONAL: return 'bg-slate-100 text-slate-700 border-slate-200 hover:bg-slate-200';
  ...
  const renderCalendarDays = () => {
    ...
    for (let i = 0; i < firstDay; i++) {
      days.push(
        <div key={`empty-${i}`} className="bg-slate-50/30 border-b border-r border-slate-100 min-h-[120px]"></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div key={day} className={`border-b border-r border-slate-100 p-2 min-h-[120px] transition-colors hover:bg-slate-50 flex flex-col ${isToday ? 'bg-blue-50/30' : ''}`}>
          <div className="flex justify-between items-start mb-2">
            ...
            {daySchedules.length > 0 && (
              <span className="text-[10px] text-slate-400 font-medium">{daySchedules.length}건</span>
            )}
          </div>
            ...
            {daySchedules.map(schedule => (
              ...
              >
                <div className="flex items-center gap-1">
                    {schedule.startTime && <span className="font-mono text-[10px] opacity-80">{schedule.startTime}</span>}
                    <span className="font-semibold truncate">{schedule.title}</span>
                </div>
                <div className="text-[10px] opacity-80 truncate">{schedule.userName}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    if (remainingCells < 7) {
       for (let i = 0; i < remainingCells; i++) {
        days.push(
            <div key={`empty-end-${i}`} className="bg-slate-50/30 border-b border-r border-slate-100 min-h-[120px]"></div>
        );
       }
    }
        ...
        <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center bg-white border border-slate-200 rounded-lg shadow-sm">
                    <button onClick={prevMonth} className="p-1.5 hover:bg-slate-50 text-slate-600 rounded-l-lg border-r border-slate-100 transition-colors">
                        <ChevronLeft size={20} />
                    </button>
                    <button onClick={goToToday} className="px-3 py-1.5 text-sm font-medium hover:bg-slate-50 text-slate-600 transition-colors">
                        오늘
                    </button>
                    <button onClick={nextMonth} className="p-1.5 hover:bg-slate-50 text-slate-600 rounded-r-lg border-l border-slate-100 transition-colors">
            ...
            <div className="flex gap-2 text-sm text-slate-500 hidden lg:flex">
                {Object.values(ScheduleType).map((type) => (
                    <div key={type} className="flex items-center gap-1">
                        <div className={`w-2 h-2 rounded-full ${getTypeColorStyles(type).split(' ')[0].replace('bg-', 'bg-').replace('-100', '-400')}`}></div>
                        <span>{type}</span>
                    </div>
                ))}
            </div>
      ...
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
          {['일', '월', '화', '수', '목', '금', '토'].map((day, i) => (
             <div key={day} className={`py-3 text-center text-sm font-semibold ${i === 0 ? 'text-red-500' : 'text-slate-600'}`}>{day}</div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-full max-w-lg border border-slate-100 animate-scale-in relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">일정 종류</label>
                <select 
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">설명/장소</label>
